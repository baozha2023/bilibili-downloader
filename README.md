...
### v5.8.2
- **新增**：视频去水印和逐帧获取页面支持预览图缩放与平移操作，提升交互体验。
- **新增**：视频去水印功能新增 "Simple Lama (AI)" 模式，提供更智能的去水印效果。
- **新增**：我的账号页面新增“稍后再看”列表，支持右键菜单操作。
- **优化**：全面重构UI代码，提升代码可读性与维护性。
- **优化**：统一项目代码格式与规范。

### v5.8.1
- **修复**：视频分析中无法获取弹幕信息的问题。
- **修复**：设置中选择1080P 60帧/高码率时实际下载为1080P 30帧的问题。
- **优化**：统一管理视频画质、编码和音质的配置常量，提升代码维护性。
...
- 优化：更新流程逻辑优化，更新时保留 `downloads` 和 `bangumi` 文件夹
- 优化：构建脚本输出文件夹包含版本号
- 优化：合集下载中支持设置中“下载完成后操作”的执行逻辑

### v5.7.6
- 修复：设置中重试次数和重试间隔不生效的bug
- 修复：热门视频中选择不同数量的页数，实际查询只查询20条的bug

### v5.7.5
- 新增：启动时自动检测Gitee新版本提醒
- 新增：启动画面及进度条显示
- 新增：视频编辑Tab增加音视频合并(AV Merge)功能
- 优化：全屏化布局适配
- 优化：代码结构清理和优化
...
- 新增：版本切换进度条显示，提升更新体验
- 优化：视频合并后自动选中目标文件
- 修复：版本切换失败及显示不存在版本的Bug
- 优化：代码结构清理和优化

### v5.7.3
- 新增：收藏夹弹窗新增批量下载按钮，一键获取收藏夹所有视频
- 优化：番剧下载更名为合集下载，功能定位更准确
- 新增：合集下载支持批量输入BV号（支持逗号/分号分隔），自动创建虚拟合集
- 优化：代码结构清理和优化，提升稳定性

### v5.7.2
- 新增：用户查询CRC32反推本地缓存，提升反查速度
- 新增：番剧下载支持BV号输入，自动检测合集并跳转
- 新增：视频下载支持合集检测，提示跳转至番剧下载批量获取
- 优化：重构下载参数构建逻辑，减少代码冗余

### v5.7.1
- 修复：视频分析中相关视频推荐未显示的问题
- 优化：视频编辑各功能页面样式统一
- 优化：代码结构清理和优化
